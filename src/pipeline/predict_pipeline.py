    ...
    class PredictionPipeline:
        ...
        def get_predicted_dataframe(self, input_dataframe_path: pd.DataFrame):
            try:
                ...
                trarget_column_maping = {0: 'bad', 1: 'good'}
                input_dataframe[prediction_column_name] = input_dataframe[prediction_column_name].map(trarget_column_maping)

                os.makedirs(self.prediction_pipeline_config.prediction_output_dirname, exist_ok=True)
                input_dataframe.to_csv(self.prediction_pipeline_config.model_file_path, index=False)
